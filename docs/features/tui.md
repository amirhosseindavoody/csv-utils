...
| Row gutter (`▸`/`▐`) **right-click** | Open context menu: **Select**, **Add to Selection** / **Remove from Selection**, **Hide** / **Unhide**, **Pin** / **Unpin** |
...
| Column list **right-click** | Open context menu: **Select**, **Add to Selection** / **Remove from Selection**, **Hide** / **Unhide**, **Info**, **Pin** / **Unpin** |
...
Context menu keys (after right-click on the column sidebar or row gutter): **↑/↓** move highlight, **Enter** activate, **Esc** / **q** dismiss; left-click an item to activate. **Ctrl+right-click** adds the column or row to a multi-selection (same as Ctrl+click). **Select** applies single-column or single-row selection; other actions preserve an existing multi-select and include the clicked column or row in bulk operations. **Add to Selection** / **Remove from Selection** builds a multi-selection through the menu alone — no modifier key required (some terminals or multiplexers don't forward Ctrl on mouse clicks): the first use seeds the previously-selected column/row alongside the clicked one, and repeated use on other columns/rows keeps extending (or shrinking) the selection.
